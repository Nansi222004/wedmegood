...
const Shortlist = () => {
  ...
  const fetchShortlist = async () => {
    try {
      ...
      if (res.success && res.data) {
        setShortlistedVendors(res.data.favorites || []);
      }
    } catch (err) {
      console.error('Failed to load shortlist:', err);
      showToast('Failed to load shortlist', 'error', 2000);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const removeFromShortlist = async (vendorId) => {
    try {
      await userApi.removeFavorite(vendorId);
      setShortlistedVendors(prev => prev.filter(v => (v.id || v.vendorId) !== vendorId));
      showToast('Vendor removed from shortlist', 'info', 2000);
    } catch (err) {
      console.error('Failed to remove from shortlist:', err);
      showToast('Failed to remove from shortlist', 'error', 2000);
    }
  };
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs" style={{ color: theme.semantic.text.secondary }}>
                {shortlistedVendors.length} shortlisted vendors
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="px-4 py-4">
        {shortlistedVendors.length === 0 ? (
          <div className="text-center py-16">
            <Icon name="bookmark" size="xl" style={{ color: theme.colors.primary[300] }} className="mx-auto mb-4" />
            <h3 className="text-lg font-bold mb-2" style={{ color: theme.semantic.text.primary }}>
              No Shortlisted Vendors
            </h3>
            <p className="text-sm mb-6" style={{ color: theme.semantic.text.secondary }}>
              Bookmark vendors to compare services and pricing
            </p>
            <Button
              ...
              return (
                ...
                >
                  <div className="relative">
                    ...
                    >
                      <Icon name="bookmark" size="xs" style={{ color: theme.colors.primary[500] }} />
                    </button>
                  </div>
